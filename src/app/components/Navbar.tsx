...
function Navbar() {
  ...
  return (
    <motion.div 
    initial={{backgroundColor:"zinc-900"}}
    animate={{
      backgroundColor: isScrolled ? 'transparent' : 'zinc-900',
    }}
    transition={{ duration: 0.3 }}
    className= {`fixed z-[999]  w-full px-12 py-6 font-["Neue Montreal"] flex justify-between items-center ${isScrolled ? 'backdrop-blur-sm' : ''
}`}>
      <div className='logo'>
      ...
      <div className="links flex gap-10">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index)=>(
            <a key={index} className={`text-sm capitalize font-light ${index ===4 && 'ml-32'}`}>{item}</a>
        ))}
      </div>
    </motion.div>
  )
